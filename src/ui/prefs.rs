...
pub fn present(
    ...
    let durations = adw::PreferencesGroup::builder()
        .title(gettext("Durations"))
        .build();

    let cfg = config.borrow().clone();
    let focus = spin_row(
        gettext("Focus"),
        gettext("Minutes per focus session"),
        1.0,
        180.0,
        cfg.focus_minutes,
    );
    let short_break = spin_row(
        gettext("Short Break"),
        gettext("Minutes per short break"),
        1.0,
        60.0,
        cfg.short_break_minutes,
    );
    let long_break = spin_row(
        gettext("Long Break"),
        gettext("Minutes per long break"),
        1.0,
        120.0,
        cfg.long_break_minutes,
    );
    let cadence = spin_row(
        gettext("Sessions Until Long Break"),
        gettext("Focus sessions before a long break"),
        1.0,
        12.0,
    ...
    let auto_breaks = switch_row(
        gettext("Auto-Start Breaks"),
        gettext("Begin breaks as soon as a focus session ends"),
        cfg.auto_start_breaks,
    );
    let auto_focus = switch_row(
        gettext("Auto-Start Focus"),
        gettext("Begin the next focus session as soon as a break ends"),
        cfg.auto_start_focus,
    );
    let sound = switch_row(
        gettext("Sound"),
        gettext("Play a sound when a session ends"),
        cfg.play_sound,
    );
    ...
    let autostart_row = switch_row(
        gettext("Start on Login"),
        gettext("Launch Blue Time in the background when you log in"),
...
fn spin_row(title: String, subtitle: String, min: f64, max: f64, value: u32) -> adw::SpinRow {
...
fn switch_row(title: String, subtitle: String, active: bool) -> adw::SwitchRow {
